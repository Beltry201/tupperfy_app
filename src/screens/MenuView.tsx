...
const MenuView = () => {
  ...
  const [savedEvent, setSavedEvent] = useState(null); // Estado para almacenar el evento guardado
  ...
  const handleSaveEvent = () => {
    ...
    setSavedEvent(eventDetails); // Guardar el evento
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        ...
        {savedEvent && (
          <View style={styles.savedEventContainer}>
            ...
            <Text>{`Hora de entrega: ${savedEvent.deliveryTime}`}</Text>
            <Text>{`Método de Pago: ${savedEvent.paymentMethod}`}</Text>
            <Text>{`Costo Final: ${savedEvent.dishes.length * 10 + 5 + parseFloat(calculateTip())}`}</Text>
